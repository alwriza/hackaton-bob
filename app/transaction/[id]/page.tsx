...
export default function TransactionPage() {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    // Frontend (UX) Check
    const { score, isSafe } = calculateRiskScore(newMessage);

    if (!isSafe) {
      setSafetyWarning('Нельзя делиться контактами. Общайтесь внутри платформы.');
      // For HIGH risk, also notify parent
      if (score >= 50) {
        await notifyParent('SAFETY', `Попытка делиться контактами (Risk Score: ${score}) в чате сделки`, user.id);
      }
      return;
    }

    if (score >= 20) {
      setSafetyWarning('Внимание: Обнаружены подозрительные слова или мат. Будьте вежливы!');
      // Non-blocking: still send but show warn
    }

    setIsSendingMsg(true);
    try {
      // Backend (Real protection) Check -> Send message through API
      ...
      if (!res.ok) {
        ...
        setSafetyWarning(errData.error || 'Ошибка отправки сообщения сервером');
        return;
      }
      ...
      setSafetyWarning('Произошла ошибка сети');
    } finally {
        ...
        <div className="flex flex-1 gap-4 overflow-hidden">
          ...
          <div className="flex-1 flex flex-col bg-white rounded-[32px] border border-border overflow-hidden shadow-sm">
            ...
            {(isFrozen || isCompleted || isDisputed || isPendingReview) && (
              <div className={`px-6 py-3 flex items-center gap-2 text-sm font-bold ${isFrozen ? 'bg-destructive/10 text-destructive border-b border-destructive/20' :
                ...
                return (
                  <div key={msg.id} className={`flex ${fromMe ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-1`}>
                    <div className="max-w-[72%] space-y-1">
                      <div className={`px-5 py-3.5 rounded-[24px] shadow-sm ${fromMe
