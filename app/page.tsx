...
import {
  ...
  CheckCircle2, Users, CreditCard, UserCircle, Loader2, Zap
} from 'lucide-react';
  ...
  return (
    <div className="space-y-24 pb-20 overflow-x-hidden">
      ...
      <section className="relative pt-12 md:pt-20">
        ...
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[500px] bg-secondary/5 rounded-full blur-[120px] animate-glow pointer-events-none" />
        <div className="absolute top-20 right-0 w-64 h-64 bg-accent/5 rounded-full blur-3xl animate-float-slow pointer-events-none" />
        <div className="absolute bottom-0 left-0 w-48 h-48 bg-primary/5 rounded-full blur-3xl animate-float pointer-events-none" style={{ animationDelay: '1s' }} />
        ...
        <div className="container mx-auto px-4 lg:px-6 relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-8 items-center">
            ...
            <div className="space-y-8 relative z-10">
              <div
                className="inline-flex items-center gap-3 bg-secondary/10 text-secondary px-5 py-2 rounded-2xl text-xs font-black uppercase tracking-widest border border-secondary/20 shadow-sm animate-fade-up"
              ...
              <p
                className="text-xl text-muted-foreground font-medium max-w-xl leading-relaxed animate-fade-up"
              ...
              <div
                className="flex items-center gap-8 pt-4 border-t border-border/40 w-fit pr-10 animate-fade-up"
            ...
            <div className="relative lg:h-[600px] flex items-center justify-center">
              {/* Glow behind image */}
              <div className="absolute inset-0 bg-primary/5 rounded-[60px] blur-3xl rotate-3 animate-glow" />

              <div className="relative w-full h-full max-w-xl lg:max-w-none rounded-[48px] overflow-hidden border border-border/40 shadow-premium animate-fade-up" style={{ animationDelay: '200ms' }}>
                <img
              ...
              <div className="absolute -bottom-4 -left-4 glass p-5 rounded-[28px] shadow-premium animate-float" style={{ animationDelay: '0.5s' }}>
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 bg-secondary/20 rounded-2xl flex items-center justify-center text-secondary">
                    <ShieldCheck size={22} />
                  </div>
                  <div>
                    <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest leading-none mb-1">Защита</p>
                    <p className="text-sm font-black tracking-tight">Escrow активен</p>
                  </div>
                </div>
              </div>

              {/* Floating: Zap badge */}
              <div className="absolute -top-4 -right-4 glass p-4 rounded-[24px] shadow-premium animate-float-slow">
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 bg-accent/20 rounded-xl flex items-center justify-center text-accent">
                    <Zap size={18} />
                  </div>
                  <div>
                    <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest leading-none mb-0.5">Быстрый старт</p>
      ...
      <section className="container mx-auto px-4 lg:px-6 space-y-12">
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[1, 2, 3].map((i) => (
              <div key={i} className="aspect-[1/1.2] skeleton rounded-[40px]" />
            ))}
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {services.map((service, idx) => (
              <Link
                ...
                className="premium-card rounded-[40px] flex flex-col group h-full animate-fade-up"
      ...
      <section className="container mx-auto px-4 lg:px-6">
        <div className="bg-primary rounded-[64px] p-8 md:p-16 lg:p-24 text-white relative overflow-hidden shadow-2xl shadow-primary/20">
          ...
          <div className="absolute -top-24 -right-24 w-64 h-64 bg-secondary/10 rounded-full blur-3xl animate-glow" />
          <div className="absolute -bottom-16 -left-16 w-48 h-48 bg-accent/5 rounded-full blur-3xl animate-float-slow" />
          ...
          <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div className="space-y-10">
              ...
              <div className="space-y-8">
                {[
                  { icon: CreditCard, title: 'Защищенная оплата', desc: 'Система эскроу удерживает средства до подтверждения результата. Прямые выплаты на карту Казахстана.' },
                  { icon: Users, title: 'Родительский контроль', desc: 'Прозрачный мониторинг доходов и сделок через привязанный аккаунт родителя.' },
                  { icon: CheckCircle2, title: 'Trust Score', desc: 'Расти профессионально. Каждый отзыв повышает твой рейтинг и доступ к VIP-заказам.' },
                ].map((item, i) => (
                  <div
                    key={i}
                  ...
                  >
                    <div className="w-16 h-16 bg-white/10 rounded-2xl flex items-center justify-center shrink-0 border border-white/10 shadow-lg hover:bg-white/20 transition-colors duration-300">
              ...
              <div className="absolute inset-0 bg-white/5 rounded-[64px] border border-white/10 backdrop-blur-xl p-12 flex flex-col justify-center space-y-10">
                <div className="space-y-8">
                  <div className="flex items-center gap-6">
                    <div className="w-16 h-16 bg-success/20 rounded-full flex items-center justify-center text-success border border-success/20 animate-glow">
      ...
      {/* ── Trust Strip ── */}
      <section className="container mx-auto px-4 lg:px-6">
        <div className="flex flex-wrap justify-center gap-12 lg:gap-24 opacity-50 hover:opacity-100 transition-all duration-700">
          {[
            ...
            <div key={i} className="flex items-center gap-4 group">
              <div className="w-14 h-14 bg-slate-50 border border-border/40 rounded-full flex items-center justify-center group-hover:bg-primary/5 group-hover:border-primary/20 transition-all duration-300">
